...
- [x] 2.2 Add a one-line note to the command's `setDescription()` / help output documenting the
      ...
      to expect.
- [x] 2.3 `php -l` + `composer check:strict` on the changed files.

## 3. Verify

- [x] 3.1 Run `occ humaniq:rules:audit` against a register with zero mandatory violations — confirm
      exit code `0`.
- [x] 3.2 Run it again against register data seeded/edited to trigger at least one mandatory
...
- [x] 3.3 Confirm `occ humaniq:rules:seed-testdata` (a separate command, unaffected by this change)
      still runs and exits `0` as before.

## 4. Follow-up (tracked, not built here)

- [x] 4.1 File a cross-cutting note (fleet-level, in the parent review's report — not a hydra ADR
      ...
      only the hook, never a second evaluator. Filed as ConductionNL/openregister#3493.
- [x] 4.2 Any enforcement implementation that follows honours the standing constraint recorded in
